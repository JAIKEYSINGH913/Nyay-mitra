...
export default function NyayAuditPage() {
  ...
  const runAudit = async () => {
    ...
    try {
      const response = await fetch(`https://nyay-python-gateway.fly.dev/api/audit/verify`, {
        method: "POST",
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    try {
      const response = await fetch(`https://nyay-python-gateway.fly.dev/api/audit/upload-pdf`, {
        method: "POST",
        body: formData
      });
